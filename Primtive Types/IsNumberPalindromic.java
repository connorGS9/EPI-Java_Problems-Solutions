...
public class IsNumberPalindromic {
  ...
  public static boolean isPalindromeNumber(int x) {
	...
	//We need to continually compare the last and first digit of the number to ensure they are the same 
    	//We know that x % 10 will give us the ones place, but how to get the largest digit place? We can find the total number of digits by using rules of logs -> #digits = Floor(log10(x)) + 1
	...
	if (x <= 0) {
		return x == 0; //0 is a palindrome, all negative numbers are not since the (-) sign counts and there's always only one on the left
	}
	...
	for (int i = 0; i < (numDigits / 2); i++) { //Iterate over half the digits since we are comparing left and right in same iteration
		...
		MASK /= 100; //Lower the mask by 1 power of 10 per number removed (always 2) -> 10^2 = 100
	}
